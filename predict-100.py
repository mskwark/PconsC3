...
# Minimal sequence separation between reported contacts
minimalseparation = 5
...
# If you set it to 0
treefraction = 0.1
if not (0 < treefraction <= 1):
    sys.stderr.write('I cannot use {:5.1f}% of trees...\n'.format(treefraction*100))
    sys.exit(0)
...
finalscores = zip(p , Yp)
finalscores.sort(reverse=True)
of = open(outfile + '.RR', 'w')
of.write('PFRMAT RR\nMODEL 1\nMETHOD PconsC3-Julia. \nUsing {:5.1f}% of estimators and up to {:7d}s time per layer\nREMARK Execution time: {:3.1f}s. Reporting only contacts |i-j| >= {:d}\n'.format(100*treefraction, maxtime, time.time() - firststart, minimalseparation))
for s in finalscores:
    if abs(s[1][1] - s[1][0]) < minimalseparation:
        continue
    of.write('{:<4d} {:<4d} 0 8 {:7.5f}\n'.format(s[1][0], s[1][1], s[0]))
of.write('END\n')
